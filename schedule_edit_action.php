...
if($_REQUEST['command'] === "delete") {
    ...
    $result = $sql_select->fetch();
    if($result !== false) {
        $sql_update = $pdo -> prepare('delete from schedule where member_id = ? and schedule_year =? and schedule_month = ? and schedule_day = ?');
        $sql_update -> execute([$_REQUEST['member_id'], $_REQUEST['year'], $_REQUEST['month'], $_REQUEST['day']]);
    }
}
